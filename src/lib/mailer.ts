...
const FROM = process.env.EMAIL_FROM || "no-reply@clases-particulares.com";
...
export function sendVerificationEmail(email: string, token: string) {
  ...
  return sendEmail(
    email,
    "Verifica tu email — ClasesParticulares",
...
export function sendPasswordResetEmail(email: string, token: string) {
  ...
  return sendEmail(
    email,
    "Recupera tu contraseña — ClasesParticulares",
...
export function sendNewMessageEmail(
  ...
  return sendEmail(
    email,
    `Nuevo mensaje de ${senderName} — ClasesParticulares`,
